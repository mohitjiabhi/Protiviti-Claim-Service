...
export const handleClaimUpload = async (req, res) => {
    ...
    try {
        ...
        const response = await axios.post("http://duplicate-check:8000/check", {content: baseString});
        if(response.data.stdout) {
            ...
            return res.status(500).json({
                ...
                claim_number,
                error: response.data.stderr || `unknown error with exit code ${response.data.exit_code}`
            });
        }
