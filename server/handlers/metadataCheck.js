...
const name = 'Metadata Check'
const folders = ["Input Files", "Image Files", "Excel Files"]
export async function metadataCheck(requestId, res) {
  const metadata = {};
  ...
  console.log(`Metadata for request ${requestId}: metadata check`, metadata);
  ...
  const scriptPath = path.join(process.cwd(), "scripts", checksMappingWithPythonFile.metadataChk);
